...
- ☐ 论文中是否出现了明显的语法错误（如 the the）、错别字等低级错误？（如有，请修正）
- ☐ 英文括号与单词之间是否添加了空格？（如无，请添加）
- ☐ 全文中相同符号的格式（是否斜体、是否加粗等）是否存在不一致？（如有，请统一）
- ☐ 全文中术语/符号的使用是否保持一致，如 `BLSTM` vs. `BiLSTM`，`$x_t$` vs. `$x(t)$` 等
- ☐ 已经定义过的符号/术语是否存在重复定义/多次定义的情况？（如有，请删除重复定义）
- ☐ 论文中是否存在原样粘贴的图片、文字？（如有，请重新绘制和改写）
...
4. 推荐格式
    + 标量用小写斜体字母表示，如 `$x$`, `$y$`, `$z$`
    + 向量用小写粗体字母表示，如 `$\mathbf{x}$`, `$\mathbf{y}$`, `$\mathbf{z}$`
    + 矩阵用大写粗体字母表示，如 `$\mathbf{X}$`, `$\mathbf{Y}$`, `$\mathbf{Z}$`
    + 公式中的说明性文字用非斜体字母表示，如 `$\mathbf{X}_{\text{noisy}}$`
    + 函数名用非斜体字母表示，如 `$\sin$`, `$\cos$`, `$\log$`, `$\operatorname{Softmax}$`
